...
namespace DairyMVC.Controllers
{
    public class StaffController : Controller
    {
        ...
        {
            var orignalRecord = (from m in instance.tblStaffs where m.id == staffToDelete.id select m).First();
            if (!ModelState.IsValid)

                instance.Entry(orignalRecord).CurrentValues.SetValues(staffToDelete);
            instance.SaveChanges();
