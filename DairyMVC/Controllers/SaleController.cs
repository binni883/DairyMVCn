...
namespace DairyMVC.Controllers
{
    public class SaleController : Controller
    {
        ...
        {
            var orignalRecord = (from m in instance.tblSales where m.id == saleToDelete.id select m).First();
            if (!ModelState.IsValid)

                instance.Entry(orignalRecord).CurrentValues.SetValues(saleToDelete);
            instance.SaveChanges();
