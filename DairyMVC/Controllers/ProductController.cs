...
namespace DairyMVC.Controllers
{
    public class ProductController : Controller
    {
        ...
        {
            var orignalRecord = (from m in instance.tblProducts where m.id == productToDelete.id select m).First();
            if (!ModelState.IsValid)

                instance.Entry(orignalRecord).CurrentValues.SetValues(productToDelete);
            instance.SaveChanges();
